...
async function gamification() {
    ...
    async function calculateScore() {
        let response = await chrome.runtime.sendMessage({ action: 'getTokenAndId' }),
            token = response?.token || await getFromStorage('token', 'local'),
            userId = response?.userId || await getFromStorage('user-id', 'local')
        console.info("Received user token and user ID from " + (response ? "service worker." : "stored data."))
